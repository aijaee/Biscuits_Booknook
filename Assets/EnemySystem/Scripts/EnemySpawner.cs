...
public class EnemySpawner : MonoBehaviour
{
    ...
    [Header("Room Spawning Options")]
    [SerializeField] private bool allowSpawnRoomEnemies = false;
    [SerializeField] private bool allowPuzzleRoomEnemies = false;

    private List<GameObject> enemyInstances = new List<GameObject>();
    private HashSet<Vector2Int> validSpawnTiles = null;
    private Dictionary<Vector2Int, RoomData> floorToRoom = null;
    ...
    public void SetFloorToRoomMapping(Dictionary<Vector2Int, RoomData> mapping)
    {
        floorToRoom = mapping;
    }

    public void SetValidSpawnTiles(HashSet<Vector2Int> tiles)
    {
        validSpawnTiles = tiles;
    }

    public List<Vector2Int> GetRandomWalkableTiles(int count, int wallBuffer = 1)
    {
        var tiles = new List<Vector2Int>();
        if (gridManager == null || gridManager.GetGrid() == null || validSpawnTiles == null)
            return tiles;
        ...
        foreach (var pos in validSpawnTiles)
        {
            if (!grid[pos.x, pos.y].walkable)
                continue;

            if (floorToRoom != null && floorToRoom.TryGetValue(pos, out RoomData room))
            {
                if ((room.Type == RoomType.Spawn && !allowSpawnRoomEnemies) ||
                    (room.Type == RoomType.Puzzle && !allowPuzzleRoomEnemies))
                    continue;
            }
